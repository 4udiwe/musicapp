...
func (r *Repository) Create(ctx context.Context, album entity.Album) (id int64, err error) {
	query, args, err := r.builder.
		...
		ToSql()
	if err != nil {
		return 0, fmt.Errorf("%w: failed to build query: %v", ErrDatabase, err)
	}
		...
		if errors.As(err, &pgErr) {
			if pgErr.Code == "23505" { // unique_violation
				return 0, fmt.Errorf("%w: album with title '%s' and artist '%s' already exists",
					ErrAlbumAlreadyExists, album.Title, album.Artist)
			}
		}
		if errors.Is(err, pgx.ErrNoRows) {
			return 0, fmt.Errorf("%w: no returned id after insert", ErrDatabase)
		}
		return 0, fmt.Errorf("%w: failed to execute query: %v", ErrDatabase, err)
	}
	return id, nil
}
...
func (r *Repository) FindAll(ctx context.Context) (albums []entity.Album, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: failed to build query: %v", ErrDatabase, err)
	}
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) {
			return nil, fmt.Errorf("%w: database error code %s: %v",
				ErrDatabase, pgErr.Code, pgErr.Message)
		}
		return nil, fmt.Errorf("%w: failed to execute query: %v", ErrDatabase, err)
	}
	defer rows.Close()
...
func (r *Repository) FindById(ctx context.Context, id int64) (album entity.Album, err error) {
	...
	if err != nil {
		return entity.Album{}, fmt.Errorf("%w: failed to build query: %v", ErrDatabase, err)
	}
	err = r.pool.QueryRow(ctx, query, args...).Scan(&album.ID, &album.Title, &album.Artist, &album.Price)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return entity.Album{}, fmt.Errorf("%w: album with id '%d' not found", ErrAlbumNotFound, id)
		}
		...
		return entity.Album{}, fmt.Errorf("%w: failed to execute query: %v", ErrDatabase, err)
	}
...
func (r *Repository) Delete(ctx context.Context, id int64) error {
	...
	if err != nil {
		return fmt.Errorf("%w: failed to build delete query: %v", ErrDatabase, err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("%w: failed to execute delete query: %v", ErrDatabase, err)
	}

	// Проверяем, была ли удалена хотя бы одна запись
	if result.RowsAffected() == 0 {
		return fmt.Errorf("%w: album with id %d not found", ErrAlbumNotFound, id)
	}
